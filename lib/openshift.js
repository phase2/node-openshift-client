...
OpenShift.prototype.showUser = function(resultCallback) {
  this._request(this.options('user?nolinks=true'),resultCallback);
};

OpenShift.prototype.listDomains = function(resultCallback){
  this._request(this.options('domains?nolinks=true'),resultCallback);
};
...
OpenShift.prototype.listApplications = function(domain_name , resultCallback){
  var rest_url_fragment = 'domains/'+ domain_name+ '/applications?nolinks=true';
...
OpenShift.prototype.listApplicationsAndCartridges = function(domain_name , resultCallback){
  var rest_url_fragment = 'domains/'+ domain_name+ '/applications?include=cartridges&nolinks=true';
  this._request(this.options(rest_url_fragment), resultCallback);
};
...
OpenShift.prototype.listAliases = function(domain_name, app_name, resultCallback){
  var rest_url_fragment = 'domains/'+ domain_name+ '/applications/' + app_name + '/aliases?nolinks=true';
  this._request(this.options(rest_url_fragment), resultCallback);
};

OpenShift.prototype.createAlias = function(domain_name, app_name, alias_name, resultCallback){
  var rest_url_fragment = 'domains/' + domain_name + '/applications/' + app_name + '/aliases';
  this._request(this.options(rest_url_fragment, {id: alias_name}, 'POST'), resultCallback);
};

OpenShift.prototype.deleteAlias = function(domain_name, app_name, alias_name, resultCallback){
  var rest_url_fragment = 'domains/' + domain_name + '/applications/' + app_name + '/aliases/' + alias_name;
  this._request(this.options(rest_url_fragment, null, 'DELETE'), resultCallback);
};
...
OpenShift.prototype._request = function(options, callback) {
  ...
  if (request_options.method == 'POST') {
    ...
    if (typeof request_options.form == 'string' || request_options.form instanceof String) {
      ...
      data = JSON.stringify(request_options.form);
    }
