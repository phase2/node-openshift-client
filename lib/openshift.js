...
function OpenShift(options){
  ...
  this.verify_ssl = options.verify_ssl ? options.verify_ssl : true;
}

function options(options, rest_url_fragment, data, method){
  var optionsObj = {
    method : method || 'GET',
    url : options.base_url + rest_url_fragment,
    headers : {
      'Accept' : 'application/json',
      'Authorization' : 'Basic ' + options.credentials
    },
    strictSSL : options.verify_ssl,
    form : data
  };
  return optionsObj;
}
...
OpenShift.prototype.authorizationToken = function(resultCallback){
  this._request(options(this.options , 'user/authorizations', {scope:'session'},'POST'),resultCallback);
};

OpenShift.prototype.showUser = function(resultCallback) {
  this._request(options(this.options , 'user'),resultCallback);
};

OpenShift.prototype.listDomains = function(resultCallback){
  this._request(options(this.options , 'domains'),resultCallback);
};

OpenShift.prototype.createDomain = function(domain_name , resultCallback){
  this._request(options(this.options , 'domains' , {id:domain_name},'POST'),resultCallback);
};

OpenShift.prototype.viewDomainDetails = function(domain_name, resultCallback){
  this._request(options(this.options , 'domains/'+domain_name),resultCallback);
};

OpenShift.prototype.updateDomain = function(old_domain_name , new_domain_name,resultCallback){
  this._request(options(this.options , 'domains/'+old_domain_name , {id : new_domain_name},'PUT'), resultCallback);
};

OpenShift.prototype.deleteDomain = function(domain_name , resultCallback){
  var force = true;
  this._request(options(this.options , 'domains/'+domain_name, {force : force},'DELETE'), resultCallback);
};
...
OpenShift.prototype.listApplications = function(domain_name , resultCallback){
  ...
  this._request(options(this.options ,rest_url_fragment), resultCallback);
};
...
OpenShift.prototype.createApplication = function(domain_name, app_options, resultCallback){
  var rest_url_fragment = 'domains/'+domain_name+'/applications';
  this._request(options(this.options , rest_url_fragment , app_options,'POST') , resultCallback);
};
...
OpenShift.prototype.startApplication = function(domain_name,app_name, resultCallback){
  ...
  this._request(options(this.options , rest_url_fragment,{event:'start'},'POST'),resultCallback);
};

OpenShift.prototype.stopApplication = function(domain_name,app_name, resultCallback){
  var rest_url_fragment = 'domains/'+domain_name+'/applications/'+app_name+'/events';
  this._request(options(this.options , rest_url_fragment,{event:'stop'},'POST'),resultCallback);
};

OpenShift.prototype.forceStopApplication = function(domain_name,app_name, resultCallback){
  var rest_url_fragment = 'domains/'+domain_name+'/applications/'+app_name+'/events';
  this._request(options(this.options , rest_url_fragment,{event:'force-stop'},'POST'),resultCallback);
};

OpenShift.prototype.restartApplication = function(domain_name,app_name, resultCallback){
  var rest_url_fragment = 'domains/'+domain_name+'/applications/'+app_name+'/events';
  this._request(options(this.options , rest_url_fragment,{event:'restart'},'POST'),resultCallback);
};

OpenShift.prototype.deleteApplication = function(domain_name , app_name, resultCallback){
  var rest_url_fragment = 'domains/'+domain_name+'/applications/'+app_name;
  this._request(options(this.options , rest_url_fragment,null, 'DELETE'),resultCallback);
};

OpenShift.prototype.scaleUpApplication = function(domain_name , app_name, resultCallback){
  var rest_url_fragment = 'domains/'+domain_name+'/applications/'+app_name+'/events';
  this._request(options(this.options , rest_url_fragment,{event:'scale-up'},'POST'),resultCallback);
};

OpenShift.prototype.scaleDownApplication = function(domain_name , app_name, resultCallback){
  var rest_url_fragment = 'domains/'+domain_name+'/applications/'+app_name+'/events';
  this._request(options(this.options , rest_url_fragment,{event:'scale-down'},'POST'),resultCallback);
};

OpenShift.prototype.listCartridges = function(resultCallback){
  this._request(options(this.options,'cartridges'), resultCallback);
};
...
OpenShift.prototype.addCartridge = function(domain_name,app_name, cartridge_name, resultCallback){
  ...
  this._request(options(this.options , rest_url_fragment,{cartridge: cartridge_name},'POST'),resultCallback);
};
